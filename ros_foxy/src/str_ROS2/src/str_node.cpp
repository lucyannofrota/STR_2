...
void *publish_thread(void *arg){
    ...
    while(true){
        ...
        pointCloud_s.x = pointCloud->x; pointCloud->x = NULL;
        pointCloud_s.y = pointCloud->y; pointCloud->y = NULL;
        pointCloud_s.z = pointCloud->z; pointCloud->z = NULL;
...
class pub_view : public rclcpp::Node{
  ...
  pub_view() : Node("pub_view"){
    ...
    sem_init(&sem_1,0,0); sem_init(&sem_2,0,0); sem_init(&sem_3,0,0); sem_init(&sem_4,0,1);
  ...
  ~pub_view(){
    ...
    sem_destroy(&sem_1); sem_destroy(&sem_2); sem_destroy(&sem_3); sem_destroy(&sem_4);
  }
